...
class creekRMSNorm(nn.Module):
    def __init__(self, hidden_size, eps=1e-6):
        """
        creekRMSNorm is equivalent to T5LayerNorm. 这里也是混合精度
        """
        super().__init__()
    ...
    def forward(self, hidden_states):
        ...
        variance = hidden_states.pow(2).mean(-1, keepdim=True) # 对最后一个维度平方求均值，并且保持维度个数。
        hidden_states = hidden_states * torch.rsqrt(variance + self.variance_epsilon) # self.variance_epsilon 防0
        return self.weight * hidden_states.to(input_dtype) # weight [1024] 个 1


ALL_LAYERNORM_LAYERS.append(creekRMSNorm)
...
class creekLinearScalingRotaryEmbedding(creekRotaryEmbedding):
    """creekRotaryEmbedding extended with linear scaling. Credits to the Reddit user /u/kaiokendev"""
...
class creekDynamicNTKScalingRotaryEmbedding(creekRotaryEmbedding):
    """creekRotaryEmbedding extended with Dynamic NTK scaling. Credits to the Reddit users /u/bloc97 and /u/emozilla"""
    ...
    def _init_rope(self):
        if self.config.rope_scaling is None:
            ...
            if scaling_type == "linear":
                self.rotary_emb = creekLinearScalingRotaryEmbedding(
                    self.head_dim,
                    max_position_embeddings=self.max_position_embeddings,
                ...
                self.rotary_emb = creekDynamicNTKScalingRotaryEmbedding(
                    self.head_dim,
                    max_position_embeddings=self.max_position_embeddings,
                    scaling_factor=scaling_factor,
                    base=self.rope_theta,
...
class creekFlashAttention2(creekAttention):
    """
    creek flash attention module. This module inherits from `creekAttention` as the weights of the module stays
    ...
    def forward(
        ...
        # creekFlashAttention2 attention does not support output_attentions
        ...
        # in fp32. (creekRMSNorm handles it correctly)
...
class creekDecoderLayer(nn.Module):
    def __init__(self, config: creekConfig):
        ...
        self.hidden_size = config.hidden_size
        self.self_attn = (
            creekAttention(config=config)
            if not getattr(config, "_flash_attn_2_enabled", False)
            else creekFlashAttention2(config=config)
        )
        self.mlp = creekMLP(config)
        self.input_layernorm = creekRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
        self.post_attention_layernorm = creekRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
...
class creekPreTrainedModel(PreTrainedModel):
    ...
    _no_split_modules = ["creekDecoderLayer"]
    ...
    def _set_gradient_checkpointing(self, module, value=False):
        if isinstance(module, creekModel):
...
class creekModel(creekPreTrainedModel):
    """
    Transformer decoder consisting of *config.num_hidden_layers* layers. Each layer is a [`creekDecoderLayer`]
    ...
    def __init__(self, config: creekConfig):
        ...
        self.norm = creekRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
...
class creekForCausalLM(creekPreTrainedModel):
    ...
    def forward(
        ...
        ```python
        >>> from transformers import AutoTokenizer, creekForCausalLM

        >>> model = creekForCausalLM.from_pretrained(PATH_TO_CONVERTED_WEIGHTS)
